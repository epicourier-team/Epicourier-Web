...
**Status**: � In Progress (E1-E4 Complete, E5 In Progress)  
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                            v1.3.0 Smart Cart System                          │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌─────────────────┐    ┌─────────────────┐    ┌──────────────────────────┐ │
│  │  Shopping List  │    │    Inventory    │    │  AI Recipe Suggestions   │ │
│  │                 │    │   Management    │    │    (Python Backend)      │ │
│  ├─────────────────┤    ├─────────────────┤    ├──────────────────────────┤ │
│  │ • List CRUD     │    │ • Item CRUD     │    │ • Ingredient matching    │ │
│  │ • Auto-generate │◀──▶│ • Expiration    │───▶│ • Coverage scoring       │ │
│  │ • Aggregation   │    │   tracking      │    │ • Expiration priority    │ │
│  │ • Export        │    │ • Location tags │    │ • Gemini preferences     │ │
│  └────────┬────────┘    └────────┬────────┘    └────────────┬─────────────┘ │
│           │                      │                          │               │
│           ▼                      ▼                          ▼               │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
│  │                         Supabase PostgreSQL                              ││
│  │  shopping_lists │ shopping_list_items │ user_inventory │ Ingredient     ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| Epic # | Title                          | Priority | Est. Effort | Dependencies | Status |
|--------|--------------------------------|----------|-------------|--------------|--------|
| E1     | Database Schema & Types        | P0       | 1 Sprint    | None         | ✅ Complete |
| E2     | Shopping List System           | P1       | 1.5 Sprints | E1           | ✅ Complete |
| E3     | Inventory Management           | P1       | 1.5 Sprints | E1           | ✅ Complete |
| E4     | AI Recipe Recommendations      | P1       | 1 Sprint    | E3           | ✅ Complete |
| E5     | Integration & Polish           | P2       | 1 Sprint    | E2, E3, E4   | 🚧 In Progress |

---

## 📦 Epic 1: Database Schema & Types (Foundation)
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  #75 shopping_lists schema ──┐                                   │
│           │                  │                                   │
│           ▼                  ▼                                   │
│  #76 shopping_list_items ───▶ #78 TypeScript Types               │
│           ▲                  ▲                                   │
│           │                  │                                   │
│  #77 user_inventory ────────┘                                    │
│                                                                  │
...
│           │                                        │              │
│           ▼                                        ▼              │
│  #79 Shopping List CRUD API ──────────▶ #82 Shopping List Page   │
│           │                                        │              │
│           ▼                                        ▼              │
│  #80 Auto-generate from Calendar ─────▶ #83 Item Interactions    │
│           │                                                       │
│           ▼                                                       │
│  #81 Ingredient Aggregation ──────────▶ #84 Export Feature       │
│                                                                   │
│  #85 Shopping List API Tests ◀──────── All APIs complete         │
│                                                                   │
└───────────────────────────────────────────────────────────────────┘
```
...
```
/dashboard/shopping
├── ShoppingListSelector     - Dropdown/tabs for multiple lists
├── GenerateFromCalendar     - Date range picker + generate button
├── ShoppingListView
│   ├── ListHeader           - Name, status, date range
│   ├── ItemList             - Scrollable list of items
│   │   └── ShoppingItem     - Checkbox, name, quantity, source recipe
│   └── ListActions          - Export, delete, mark complete
└── AddItemDialog            - Search ingredient, set quantity
```

**Wireframe**:
```
┌─────────────────────────────────────────────────────────────┐
│  Shopping Lists              [+ New List] [Generate from 📅] │
├─────────────────────────────────────────────────────────────┤
│  ┌──────────────────────────────────────────────────────┐   │
│  │ 📋 Week of Dec 1-7                    Status: Active │   │
│  ├──────────────────────────────────────────────────────┤   │
│  │                                                       │   │
│  │  ☐ Chicken Breast          800g    (from 2 recipes)  │   │
│  │  ☑ Olive Oil               150ml   (from 3 recipes)  │   │
│  │  ☐ Garlic                  6 cloves                  │   │
│  │  ☐ Tomatoes                4 whole                   │   │
│  │  ☐ Rice                    500g                      │   │
│  │  ...                                                 │   │
...
│  └──────────────────────────────────────────────────────┘   │
│                                                              │
│  [🖨️ Export] [✅ Mark Complete] [🗑️ Delete]                  │
└─────────────────────────────────────────────────────────────┘
```
...
├── GET - get single list with items
├── PUT - update list
├── DELETE - delete list
└── 404 for non-existent list

shopping-lists/generate/route.ts
├── Generate from valid date range
...
│           │                                                      │
│           ▼                                                      │
│  #86 Inventory CRUD API ──────────────▶ #88 Inventory Page       │
│           │                                        │             │
│           ▼                                        ▼             │
│  #87 Expiration Queries ──────────────▶ #89 Expiration Warnings  │
│                                                                   │
│  #90 Inventory API Tests ◀──────────── All APIs complete         │
│                                                                   │
└───────────────────────────────────────────────────────────────────┘
```
...
```
/dashboard/inventory
├── LocationTabs             - Pantry | Fridge | Freezer | All
├── InventoryGrid
│   └── InventoryCard        - Ingredient, quantity, expiration badge
├── AddItemDialog            - Search ingredient, set details
├── ExpiringBanner           - Alert for soon-to-expire items
└── SearchBar                - Quick search across inventory
```

**Wireframe**:
```
┌─────────────────────────────────────────────────────────────┐
│  My Inventory                                   [+ Add Item] │
├─────────────────────────────────────────────────────────────┤
│  ⚠️ 3 items expiring soon                     [View All →]   │
├─────────────────────────────────────────────────────────────┤
│  [All] [🥫 Pantry] [❄️ Fridge] [🧊 Freezer]                  │
├─────────────────────────────────────────────────────────────┤
│  🔍 Search ingredients...                                    │
├─────────────────────────────────────────────────────────────┤
│                                                              │
...
├── GET - filter by expiration
├── POST - add new item
├── POST - upsert existing item
└── Error handling
...
├── Returns items sorted by expiration
...
## 📦 Epic 4: AI Recipe Recommendations
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Epic 3 Complete (#86, #87)                                      │
│           │                                                      │
│           ▼                                                      │
│  #91 Python: Inventory Recommend API                             │
│           │                                                      │
│           ▼                                                      │
│  #92 Python: Expiration Priority Scoring                         │
│           │                                                      │
│           ▼                                                      │
│  #93 Frontend: Suggest Recipes Button ─────▶ #94 Recommendation  │
│                                                   Modal          │
│                                                                  │
│  #95 Python API Tests ◀─────────────────── All APIs complete     │
│                                                                   │
└───────────────────────────────────────────────────────────────────┘
```
...
- [ ] Critical items (≤2 days) weighted higher
...
**Behavior**:
1. Click "🍳 Suggest Recipes" button
...
```
┌─────────────────────────────────────────────────────────────┐
│  🍳 Recipes You Can Make                              [X]   │
├─────────────────────────────────────────────────────────────┤
│  Based on your inventory, here are some suggestions:        │
│                                                              │
│  ┌──────────────────────────────────────────────────────┐   │
...
│                                                              │
│  ┌──────────────────────────────────────────────────────┐   │
│  │ 2. Vegetable Fried Rice                ★ 88% match   │   │
│  │    ✅ Uses expiring: eggs (3 days)                   │   │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Epics 2, 3, 4 Complete                                          │
│           │                                                      │
│           ▼                                                      │
│  #96 Add to Shopping List from Recipe Page                       │
│           │                                                      │
│           ▼                                                      │
│  #97 Calendar Integration (Quick Add to Cart)                    │
│           │                                                      │
│           ▼                                                      │
│  #98 Dashboard Widgets (Shopping & Inventory)                    │
│           │                                                      │
│           ▼                                                      │
│  #99 Sidebar Navigation Updates                                  │
│           │                                                      │
│           ▼                                                      │
...
│ 12 items remaining   │  │ 23 items tracked     │
│ 3 checked            │  │ ⚠️ 3 expiring soon   │
│ [View List →]        │  │ [View Inventory →]   │
└──────────────────────┘  └──────────────────────┘
```
...
|-------|-------|----------|--------|
| #75 | shopping_lists schema | - | 📝 |
| #76 | shopping_list_items schema | - | 📝 |
| #77 | user_inventory schema | - | 📝 |
| #78 | TypeScript types | - | 📝 |
...
| Issue | Title | Assignee | Status |
|-------|-------|----------|--------|
| #79 | Shopping list CRUD API | - | 📝 |
| #80 | Auto-generate from calendar | - | 📝 |
| #82 | Shopping List Page | - | 📝 |
...
| Issue | Title | Assignee | Status |
|-------|-------|----------|--------|
| #81 | Ingredient aggregation | - | 📝 |
| #83 | Shopping item interactions | - | 📝 |
| #84 | Export feature | - | 📝 |
| #85 | Shopping list tests | - | 📝 |
| #86 | Inventory CRUD API | - | 📝 |
...
| Issue | Title | Assignee | Status |
|-------|-------|----------|--------|
| #87 | Expiration queries | - | 📝 |
| #88 | Inventory Page | - | 📝 |
| #89 | Expiration warnings | - | 📝 |
| #90 | Inventory tests | - | 📝 |
| #91 | Python inventory recommend | - | 📝 |
| #92 | Expiration priority scoring | - | 📝 |
...
| Issue | Title | Assignee | Status |
|-------|-------|----------|--------|
| #93 | Suggest recipes button | - | 📝 |
| #94 | Recommendation modal | - | 📝 |
| #95 | Python API tests | - | 📝 |
| #96 | Add to list from recipe | - | 📝 |
| #97 | Calendar quick-add | - | 📝 |
| #98 | Dashboard widgets | - | 📝 |
| #99 | Sidebar navigation | - | 📝 |
| #100 | Integration tests | - | 📝 |
...
```
#75 → #76 → #78 → #79 → #80 → #82 → #83 (Shopping List Complete)
       ↓
#77 → #78 → #86 → #87 → #88 → #89 (Inventory Complete)
                    ↓
              #91 → #92 → #93 → #94 (AI Complete)
                              ↓
                    #96 → #97 → #98 → #100 (Integration Complete)
```
